...
autoinstall:
  ...
  late-commands:
    ...
    - curtin in-target -- apt-get install -y curl open-vm-tools-desktop open-vm-tools
    - curtin in-target --target=/target -- bash -c "apt-get update && apt-get install -y git open-vm-tools-desktop open-vm-tools"
    - curtin in-target --target=/target -- bash -c "git clone https://github.com/abvinodu2003/zsh-ubunutu-script.git && cd zsh-ubunutu-script/ZSH-working-tested && chmod +x *.* && ./setupzsh.sh"
    #- curtin in-target --target=/target -- bash -c "chmod +x /opt/yourrepo/yourscript.sh"
"@
