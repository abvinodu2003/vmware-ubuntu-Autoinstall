...
'@
    ...
    Write-Log "GRUB configuration updated successfully" -Level SUCCESS
}
...
function Build-CustomISO {
    ...
    Write-Log "Detecting boot files in extracted ISO..." -Level INFO
    
    try {
        Write-Progress -Activity "Building ISO" -Status "Diagnosing ISO structure..." -PercentComplete 10
        
        # DIAGNOSTIC: Let's see what's actually in the ISO
        Write-Log "=== ISO STRUCTURE DIAGNOSTIC ===" -Level INFO
        
        # Check top-level directories
        Write-Log "Top-level directories:" -Level INFO
        $topDirs = wsl -d $script:WSLDistribution bash -c "ls -1 '$wslExtractPath' 2>/dev/null"
        if ($topDirs) {
        ...
        # Check for isolinux directory and contents
        Write-Log "Checking isolinux directory:" -Level INFO
        $isolinuxExists = wsl -d $script:WSLDistribution bash -c "test -d '$wslExtractPath/isolinux' && echo yes || echo no"
        if ($isolinuxExists.Trim() -eq 'yes') {
            $isolinuxContents = wsl -d $script:WSLDistribution bash -c "ls -lh '$wslExtractPath/isolinux/' 2>/dev/null"
            Write-Host "Isolinux directory contents:" -ForegroundColor Green
            foreach ($line in $isolinuxContents) {
                Write-Host "  $line" -ForegroundColor Gray
            }
        } else {
            Write-Host "  isolinux directory NOT FOUND - Will add BIOS boot support" -ForegroundColor Yellow
        }
        
        # Check for boot/grub directory and contents
        Write-Log "Checking boot/grub directory:" -Level INFO
        $bootGrubExists = wsl -d $script:WSLDistribution bash -c "test -d '$wslExtractPath/boot/grub' && echo yes || echo no"
        if ($bootGrubExists.Trim() -eq 'yes') {
            $bootGrubContents = wsl -d $script:WSLDistribution bash -c "ls -lh '$wslExtractPath/boot/grub/' 2>/dev/null"
            Write-Host "boot/grub directory contents:" -ForegroundColor Green
            foreach ($line in $bootGrubContents) {
                Write-Host "  $line" -ForegroundColor Gray
            }
        } else {
            Write-Host "  boot/grub directory NOT FOUND" -ForegroundColor Red
        }
        
        # Check for EFI directory
        Write-Log "Checking EFI directory:" -Level INFO
        $efiExists = wsl -d $script:WSLDistribution bash -c "test -d '$wslExtractPath/EFI' && echo yes || echo no"
        if ($efiExists.Trim() -eq 'yes') {
            $efiContents = wsl -d $script:WSLDistribution bash -c "find '$wslExtractPath/EFI' -type f -name '*.efi' -o -name '*.img' 2>/dev/null"
            Write-Host "EFI files found:" -ForegroundColor Green
            foreach ($line in $efiContents) {
                Write-Host "  $line" -ForegroundColor Gray
            }
        } else {
            Write-Host "  EFI directory NOT FOUND" -ForegroundColor Red
        }
        
        # Search for boot files
        Write-Log "Searching for boot files across entire ISO:" -Level INFO
        $bootFiles = wsl -d $script:WSLDistribution bash -c "find '$wslExtractPath' -type f \( -name 'isolinux.bin' -o -name 'efi.img' -o -name '*.efi' -o -name 'boot.cat' \) 2>/dev/null"
        if ($bootFiles) {
            Write-Host "Boot files found:" -ForegroundColor Green
            foreach ($file in $bootFiles) {
                Write-Host "  $file" -ForegroundColor Yellow
            }
        } else {
            Write-Host "  NO BOOT FILES FOUND" -ForegroundColor Red
        }
        
        Write-Log "=== END DIAGNOSTIC ===" -Level INFO
        
        Write-Progress -Activity "Building ISO" -Status "Configuring boot support..." -PercentComplete 20
        
        # Check for existing BIOS boot (isolinux)
        ...
        # ADD BIOS BOOT SUPPORT if missing (for hybrid boot)
        if (-not $hasBiosBoot) {
            Write-Log "BIOS boot not found - adding isolinux for hybrid BIOS+UEFI support..." -Level INFO
            ...
            # These are provided by the syslinux-utils package we installed
            Write-Log "Copying BIOS boot files from system..." -Level INFO
            
            # Copy isolinux.bin
            wsl -d $script:WSLDistribution bash -c "cp /usr/lib/ISOLINUX/isolinux.bin '$wslExtractPath/isolinux/' 2>/dev/null || cp /usr/lib/syslinux/modules/bios/isolinux.bin '$wslExtractPath/isolinux/' 2>/dev/null"
            
            # Copy additional required files
            wsl -d $script:WSLDistribution bash -c "cp /usr/lib/syslinux/modules/bios/ldlinux.c32 '$wslExtractPath/isolinux/' 2>/dev/null || true"
            wsl -d $script:WSLDistribution bash -c "cp /usr/lib/syslinux/modules/bios/libcom32.c32 '$wslExtractPath/isolinux/' 2>/dev/null || true"
            wsl -d $script:WSLDistribution bash -c "cp /usr/lib/syslinux/modules/bios/libutil.c32 '$wslExtractPath/isolinux/' 2>/dev/null || true"
            wsl -d $script:WSLDistribution bash -c "cp /usr/lib/syslinux/modules/bios/vesamenu.c32 '$wslExtractPath/isolinux/' 2>/dev/null || true"
            
            # Create isolinux.cfg that matches our GRUB configuration
            $isolinuxCfg = @'
...
LABEL install
  ...
  APPEND initrd=/casper/initrd autoinstall ds=nocloud;s=/cdrom/nocloud/ --- quiet splash noprompt noshell automatic-ubiquity debian-installer/locale=en_US keyboard-configuration/layoutcode=us languagechooser/language-name=English localechooser/supported-locales=en_US.UTF-8 countrychooser/shortlist=IN --
...
'@
            ...
            # Write isolinux.cfg using base64 to avoid escaping issues
        ...
        # Method 1: Check for EFI/boot/bootx64.efi (Ubuntu 24.04 Desktop standard)
        $bootx64 = wsl -d $script:WSLDistribution bash -c "test -f '$wslExtractPath/EFI/boot/bootx64.efi' && echo exists || echo missing"
        if ($bootx64.Trim() -eq 'exists') {
            $hasUefiBoot = $true
            $efiPath = "EFI/boot/bootx64.efi"
            Write-Log "Found UEFI boot at: $efiPath" -Level SUCCESS
        }
        
        # Method 2: Check for boot/grub/efi.img (alternative)
        if (-not $hasUefiBoot) {
            $efiImg = wsl -d $script:WSLDistribution bash -c "test -f '$wslExtractPath/boot/grub/efi.img' && echo exists || echo missing"
            if ($efiImg.Trim() -eq 'exists') {
                $hasUefiBoot = $true
                $efiPath = "boot/grub/efi.img"
                Write-Log "Found UEFI boot at: $efiPath" -Level SUCCESS
            }
        }
        
        # Method 3: Check for EFI/boot/grubx64.efi (fallback)
        if (-not $hasUefiBoot) {
            $grubx64 = wsl -d $script:WSLDistribution bash -c "test -f '$wslExtractPath/EFI/boot/grubx64.efi' && echo exists || echo missing"
            if ($grubx64.Trim() -eq 'exists') {
                $hasUefiBoot = $true
                $efiPath = "EFI/boot/grubx64.efi"
                Write-Log "Found UEFI boot at: $efiPath" -Level SUCCESS
            }
        }
        
        Write-Log "Final boot support - BIOS: $hasBiosBoot, UEFI: $hasUefiBoot" -Level INFO
        
        if (-not $hasBiosBoot -and -not $hasUefiBoot) {
            Write-Log "CRITICAL: No boot files available!" -Level ERROR
            throw "Cannot create bootable ISO - no boot files found or created"
        }
        ...
        # Build hybrid BIOS+UEFI ISO (now that we've added BIOS support)
        if ($hasBiosBoot -and $hasUefiBoot) {
            Write-Log "Building HYBRID BIOS+UEFI bootable ISO" -Level SUCCESS
            Write-Log "BIOS boot: isolinux/isolinux.bin" -Level INFO
            Write-Log "UEFI boot: $efiPath" -Level INFO
            
            # Get isohdpfx.bin path (needed for hybrid boot)
            $isohdpfxPath = wsl -d $script:WSLDistribution bash -c "find /usr/lib -name 'isohdpfx.bin' 2>/dev/null | head -1"
            
            if ([string]::IsNullOrWhiteSpace($isohdpfxPath)) {
                Write-Log "isohdpfx.bin not found - using basic hybrid mode" -Level WARNING
                
                # Hybrid boot without isohdpfx.bin
                $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs " +
                              "-r -V 'Ubuntu-Autoinstall' -o '$wslOutputIsoPath' " +
                              "-J -joliet-long -cache-inodes " +
                              "-b isolinux/isolinux.bin -c isolinux/boot.cat " +
                              "-no-emul-boot -boot-load-size 4 -boot-info-table " +
                              "-eltorito-alt-boot -e '$efiPath' -no-emul-boot " +
                              "-isohybrid-gpt-basdat " +
                              ". 2>&1"
            } else {
                $isohdpfxPath = $isohdpfxPath.Trim()
                Write-Log "Using isohdpfx.bin: $isohdpfxPath" -Level INFO
        ...
        elseif ($hasUefiBoot) {
            ...
            Write-Log "Using EFI boot image: $efiPath" -Level INFO
            
            $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs " +
                          "-r -V 'Ubuntu-Autoinstall' -o '$wslOutputIsoPath' " +
                          "-J -joliet-long -cache-inodes " +
                          "-e '$efiPath' -no-emul-boot " +
                          "-isohybrid-gpt-basdat " +
                          ". 2>&1"
        }
        elseif ($hasBiosBoot) {
        ...
        Write-Log "xorriso command: $xorrisoCmd" -Level INFO
        ...
        # Show xorriso output for debugging
        if ($output) {
            Write-Log "xorriso output (first 20 lines):" -Level INFO
            $outputLines = $output -split "`n"
            $lineCount = [Math]::Min(20, $outputLines.Count)
            for ($i = 0; $i -lt $lineCount; $i++) {
                Write-Host "  $($outputLines[$i])" -ForegroundColor Gray
            }
        ...
        if (-not (Test-Path -LiteralPath $OutputIsoPath)) {
            Write-Log "ISO file was not created" -Level ERROR
        ...
        if ($isoFileInfo.Length -eq 0) {
            Write-Log "ISO file is empty" -Level ERROR
        ...
        Write-Log "ISO created successfully: $('{0:N2}' -f $isoSize) GB in $([math]::Round($buildTime, 2))s" -Level SUCCESS
