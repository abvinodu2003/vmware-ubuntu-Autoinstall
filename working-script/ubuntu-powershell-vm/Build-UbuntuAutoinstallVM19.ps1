...
autoinstall:
  ...
  user-data:
    ...
    ntp:
        enabled: true
    # Custom Commands - keep late-commands minimal for faster OS install.
    # All package installation is handled by the firstboot post-install script.
  late-commands:
    - cp -r /cdrom/post-install /target/opt/post-install || true
    - chmod 755 /target/opt/post-install/post-install.sh || true
    - |
    ...
    - |
      cat > /target/etc/systemd/system/post-install-firstboot.service <<SVCEOF
      [Unit]
