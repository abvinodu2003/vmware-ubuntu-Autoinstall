...
autoinstall:
  ...
  late-commands:
    ...
    - curtin in-target -- apt-get install -y open-vm-tools-desktop open-vm-tools curl wget git zsh
...
# Cleanup function: unmounts ISO and deletes working/temp directory.
...
function Invoke-Cleanup {
    ...
    # 2. Delete working/temp directory
    if ($script:WorkingDir -and (Test-Path -LiteralPath $script:WorkingDir)) {
        # Preserve build log by copying to output directory first
        if ($script:LogFile -and (Test-Path -LiteralPath $script:LogFile)) {
            try {
                $logDest = Join-Path $OutputDirectory (Split-Path $script:LogFile -Leaf)
                Copy-Item -LiteralPath $script:LogFile -Destination $logDest -Force -ErrorAction SilentlyContinue
                Write-Log "Build log preserved at: $logDest" -Level INFO
            }
            catch {
                Write-Log "Could not preserve build log: $_" -Level WARNING
            }
        }
        try {
            Remove-Item -LiteralPath $script:WorkingDir -Recurse -Force
            Write-Host "[CLEANUP] Working directory removed: $script:WorkingDir" -ForegroundColor Green
        }
        catch {
            Write-Host "[CLEANUP] Failed to remove working directory: $_" -ForegroundColor Yellow
        }
    }
}
...
function Build-VMConfiguration {
    ...
    try {
        ...
        # Relocate custom ISO from working dir to VM dir so working dir can be cleaned up.
        # Safe because VMs haven't been started yet at this point.
        $vmDir = Join-Path $OutputDirectory $vmName
        $newIsoPath = Join-Path $vmDir (Split-Path $customIsoPath -Leaf)
        Move-Item -LiteralPath $customIsoPath -Destination $newIsoPath -Force
        $vmxRaw = Get-Content -LiteralPath $vmxPath -Raw
        $vmxRaw = $vmxRaw.Replace($customIsoPath, $newIsoPath)
        Set-Content -LiteralPath $vmxPath -Value $vmxRaw -Force -NoNewline
        Write-Log "[$vmName] Relocated custom ISO to VM directory" -Level INFO
