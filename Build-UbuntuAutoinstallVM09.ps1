...
function Build-CustomISO {
    ...
    try {
        ...
        if ($isolinuxExists.Trim() -eq 'yes') {
            ...
            Write-Host "  isolinux directory NOT FOUND" -ForegroundColor Red
        }
        ...
        Write-Progress -Activity "Building ISO" -Status "Checking boot files..." -PercentComplete 20
        
        # Now check for boot files with the actual paths
        ...
        $hasUefiBoot = $efiImg.Trim() -eq 'exists'
        
        # Check alternative EFI locations
        if (-not $hasUefiBoot) {
            Write-Log "Checking alternative EFI locations..." -Level INFO
            
            # Check for bootx64.efi (common in Ubuntu)
        ...
        Write-Log "Boot support - BIOS: $hasBiosBoot, UEFI: $hasUefiBoot" -Level INFO
        
        # If still no boot files found, this is a critical error
        if (-not $hasBiosBoot -and -not $hasUefiBoot) {
            Write-Log "CRITICAL: No boot files detected in extracted ISO!" -Level ERROR
            Write-Log "This means either:" -Level WARNING
            Write-Log "1. The source ISO is corrupted" -Level WARNING
            Write-Log "2. The extraction process failed to copy boot files" -Level WARNING
            Write-Log "3. This is not a bootable Ubuntu ISO" -Level WARNING
            
            # Try to continue anyway with basic ISO
            Write-Log "Attempting to create basic (possibly non-bootable) ISO..." -Level WARNING
        }
        ...
        elseif ($hasUefiBoot) {
            Write-Log "Building UEFI-only bootable ISO with: $efiPath" -Level INFO
        ...
        Write-Log "Command: $xorrisoCmd" -Level INFO
        ...
        if (-not (Test-Path -LiteralPath $OutputIsoPath)) {
            ...
            Write-Log "xorriso output:" -Level ERROR
            if ($output) {
