...
function ConvertTo-WslPath {
    ...
    # Use wslpath with proper quoting to handle spaces
...
function Build-CustomISO {
    <#
    .SYNOPSIS
        Builds custom Ubuntu ISO with proper boot support for modern UEFI
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$ExtractPath,
    ...
    Write-Log "Detecting boot configuration for modern Ubuntu 24.04..." -Level INFO
    
    try {
        # Modern Ubuntu 24.04 uses specific boot configuration
        # We need to check for both BIOS and UEFI boot files
        
        Write-Progress -Activity "Building ISO" -Status "Checking boot files..." -PercentComplete 10
        ...
        # Debug: List what boot files actually exist
        Write-Log "Listing boot directory contents..." -Level INFO
        $bootContents = wsl -d $script:WSLDistribution bash -c "ls -la '$wslExtractPath/boot/grub/' 2>/dev/null | head -20"
        if ($bootContents) {
            Write-Log "Boot files found:" -Level INFO
            foreach ($line in $bootContents) {
                Write-Host "  $line" -ForegroundColor Gray
            }
        }
        
        $isolinuxContents = wsl -d $script:WSLDistribution bash -c "ls -la '$wslExtractPath/isolinux/' 2>/dev/null | head -10"
        if ($isolinuxContents) {
            Write-Log "Isolinux files found:" -Level INFO
            foreach ($line in $isolinuxContents) {
                Write-Host "  $line" -ForegroundColor Gray
            }
        }
        
        Write-Progress -Activity "Building ISO" -Status "Running xorriso (this will take several minutes)..." -PercentComplete 30
        
        $startTime = Get-Date
        
        # Build bootable ISO with both BIOS and UEFI support
        # FIXED: Properly quote paths with spaces
        if ($hasBiosBoot -and $hasUefiBoot) {
            Write-Log "Building hybrid BIOS+UEFI bootable ISO" -Level INFO
            
            # Full hybrid boot configuration for Ubuntu 24.04
            $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs -r -V 'Ubuntu-Autoinstall' -o '$wslOutputIsoPath' -J -joliet-long -cache-inodes -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat . 2>&1"
        }
        elseif ($hasUefiBoot) {
            Write-Log "Building UEFI-only bootable ISO" -Level INFO
            
            $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs -r -V 'Ubuntu-Autoinstall' -o '$wslOutputIsoPath' -J -joliet-long -cache-inodes -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat . 2>&1"
        }
        elseif ($hasBiosBoot) {
            Write-Log "Building BIOS-only bootable ISO" -Level INFO
            
            $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs -r -V 'Ubuntu-Autoinstall' -o '$wslOutputIsoPath' -J -joliet-long -cache-inodes -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table . 2>&1"
        }
        else {
            Write-Log "WARNING: No boot files found - attempting to find alternative boot method" -Level WARNING
            
            # Try to find any EFI file
            $efiSearch = wsl -d $script:WSLDistribution bash -c "find '$wslExtractPath' -name '*.efi' -o -name 'efi.img' 2>/dev/null | head -5"
            if ($efiSearch) {
                Write-Log "Found EFI files:" -Level INFO
                Write-Log $efiSearch -Level INFO
            }
            
            # Create basic bootable ISO
            $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs -r -V 'Ubuntu-Autoinstall' -o '$wslOutputIsoPath' -J -joliet-long . 2>&1"
        }
        
        Write-Log "Executing xorriso command..." -Level INFO
        ...
        if (-not [string]::IsNullOrWhiteSpace($output)) {
            ...
            elseif ($outputLength -gt 0) {
                Write-Log "xorriso output ($outputLength chars):" -Level INFO
                Write-Log $output -Level INFO
            }
        } else {
...
function Build-VMConfiguration {
    ...
    try {
        ...
        # FIXED: Use rsync with proper options for directories
        # --link-dest must point to an absolute path
        # Use rsync without --link-dest to avoid directory hard-link issues
        
        $startTime = Get-Date
        
        # Create destination directory
        wsl -d $script:WSLDistribution bash -c "mkdir -p '$wslVm'"
        
        # Use cp with hard links instead of rsync (more reliable for this use case)
        # -al = archive mode with hard links
        wsl -d $script:WSLDistribution bash -c "cp -al '$wslBase/.' '$wslVm/'"
        
        if ($LASTEXITCODE -ne 0) {
            Write-Log "[$vmName] Hard link copy failed, falling back to regular rsync..." -Level WARNING
