...
function Build-CustomISO {
    ...
    Write-Progress -Activity "Building ISO" -Status "Running xorriso for $VMName (this may take 2-3 minutes)..." -PercentComplete 0
    
    $wslExtractPath = ConvertTo-WslPath -WindowsPath $ExtractPath
    $wslOutputIsoPath = ConvertTo-WslPath -WindowsPath $OutputIsoPath
    
    try {
        # First check if isolinux directory exists
        $isolinuxCheck = wsl -d $script:WSLDistribution bash -c "test -d \`"$wslExtractPath/isolinux\`" && echo exists || echo missing"
        
        if ($isolinuxCheck.Trim() -ne 'exists') {
            Write-Log "Warning: isolinux directory not found, checking for ISOLINUX..." -Level WARNING
            
            # Some ISOs use uppercase ISOLINUX
            $isolinuxCheckUpper = wsl -d $script:WSLDistribution bash -c "test -d \`"$wslExtractPath/ISOLINUX\`" && echo exists || echo missing"
            
            if ($isolinuxCheckUpper.Trim() -eq 'exists') {
                # Create lowercase symlink
                wsl -d $script:WSLDistribution bash -c "ln -s \`"$wslExtractPath/ISOLINUX\`" \`"$wslExtractPath/isolinux\`""
                Write-Log "Created isolinux symlink" -Level INFO
            }
            else {
                throw "Neither isolinux nor ISOLINUX directory found in ISO"
            }
        }
        
        # Build the ISO
        $xorrisoCmd = "cd \`"$wslExtractPath\`" && xorriso -as mkisofs -r -V 'Ubuntu-24.04-Autoinstall' -o \`"$wslOutputIsoPath\`" -J -joliet-long -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat -isohybrid-apm-hfsplus -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin . 2>&1"
        
        $output = wsl -d $script:WSLDistribution bash -c $xorrisoCmd
        
        if ($LASTEXITCODE -ne 0) {
            # Check if it's just warnings or actual failure
            if ($output -match "FAILURE|ERROR") {
                Write-Log "xorriso error: $output" -Level ERROR
                throw "ISO build failed"
            }
            else {
                # Sometimes xorriso returns non-zero even on success
                Write-Log "xorriso completed with warnings (exit code $LASTEXITCODE)" -Level WARNING
            }
        }
        ...
        $isoSize = (Get-Item -LiteralPath $OutputIsoPath).Length / 1GB
        
        Write-Progress -Activity "Building ISO" -Status "Complete" -PercentComplete 100
        Write-Log "Custom ISO created: $OutputIsoPath ($('{0:N2}' -f $isoSize) GB)" -Level SUCCESS
