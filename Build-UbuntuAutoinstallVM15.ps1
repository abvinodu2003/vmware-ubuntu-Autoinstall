...
function Update-GrubConfig {
    ...
    # NOTE: ds=nocloud;s=/cdrom/ (not /cdrom/nocloud/)
...
'@
        ...
        Write-Log "Executing xorriso (3-8 minutes)..." -Level INFO
        ...
        if ($output) {
            ...
            $criticalErrors = $outputLines | Where-Object { $_ -match 'FAILURE|FATAL|aborting' }
            if ($criticalErrors) {
                Write-Log "CRITICAL ERRORS detected:" -Level ERROR
                foreach ($line in $criticalErrors) {
                    Write-Host "  $line" -ForegroundColor Red
                }
            }
            
            # Check for warnings
            $warnings = $outputLines | Where-Object { $_ -match 'WARNING' -and $_ -notmatch 'ignoring' }
            if ($warnings) {
                Write-Log "Warnings detected:" -Level WARNING
            ...
            for ($i = 0; $i -lt [Math]::Min(20, $outputLines.Count); $i++) {
                $line = $outputLines[$i]
