...
function Build-CustomISO {
    ...
    try {
        ...
        if ($isolinuxExists.Trim() -eq 'yes') {
            ...
            Write-Host "  isolinux directory NOT FOUND (This is normal for UEFI-only ISOs)" -ForegroundColor Yellow
        }
        ...
        # Check for EFI directory (alternative location)
        ...
        # Search for ANY boot-related files
        ...
        if ($bootFiles) {
            ...
            Write-Host "  NO BOOT FILES FOUND ANYWHERE" -ForegroundColor Red
        }
        
        Write-Log "=== END DIAGNOSTIC ===" -Level INFO
        
        Write-Progress -Activity "Building ISO" -Status "Detecting boot configuration..." -PercentComplete 20
        
        # FIXED: Detect boot files correctly for Ubuntu 24.04 Desktop
        # Ubuntu 24.04 Desktop is UEFI-only (no BIOS/isolinux)
        
        # Check for BIOS boot (isolinux) - not present in Desktop edition
        ...
        Write-Log "Boot support detected - BIOS: $hasBiosBoot, UEFI: $hasUefiBoot" -Level INFO
        
        if (-not $hasBiosBoot -and -not $hasUefiBoot) {
            Write-Log "CRITICAL: No boot files detected!" -Level ERROR
            Write-Log "ISO will be created but may not be bootable" -Level WARNING
        }
        ...
        # FIXED: Build proper UEFI bootable ISO for Ubuntu 24.04 Desktop
        if ($hasBiosBoot -and $hasUefiBoot) {
            Write-Log "Building hybrid BIOS+UEFI bootable ISO" -Level INFO
        ...
        elseif ($hasUefiBoot) {
            Write-Log "Building UEFI-only bootable ISO (Ubuntu 24.04 Desktop standard)" -Level INFO
            Write-Log "Using EFI boot image: $efiPath" -Level INFO
            
            # Ubuntu 24.04 Desktop UEFI boot configuration
            # This is the correct configuration for modern Ubuntu Desktop ISOs
            $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs " +
                          "-r " +
                          ...
                          "-e '$efiPath' " +
                          "-no-emul-boot " +
                          "-append_partition 2 0xef '$wslExtractPath/boot.catalog' " +
        ...
        elseif ($hasBiosBoot) {
            ...
            Write-Log "WARNING: Creating basic (non-bootable) ISO" -Level WARNING
        ...
        if ($hasUefiBoot) {
            Write-Log "ISO is UEFI bootable using: $efiPath" -Level SUCCESS
        } elseif ($hasBiosBoot) {
            Write-Log "ISO is BIOS bootable" -Level SUCCESS
        } else {
            Write-Log "WARNING: ISO may not be bootable - no boot files were detected!" -Level WARNING
        }
