...
function Invoke-WslCommand {
    ...
    try {
        $result = wsl -d $script:WSLDistribution bash -c $Command 2>&1
...
function New-SecurePasswordHash {
    ...
    try {
        ...
        $pythonCheck = Invoke-WslCommand -Command "which python3 2>/dev/null || which python 2>/dev/null" -IgnoreError
...
function Test-Prerequisites {
    ...
    try {
        ...
        Invoke-WslCommand -Command $installCmd | Out-Null
...
function Extract-UbuntuISO {
    ...
    # Extract ISO using separate commands to avoid line ending issues
    try {
        # Create mount point
        Write-Log "Creating temporary mount point..." -Level INFO
        Invoke-WslCommand -Command "mkdir -p /tmp/iso_mount" | Out-Null
        
        # Mount ISO
        Write-Log "Mounting ISO..." -Level INFO
        Invoke-WslCommand -Command "sudo mount -o loop '$wslIsoPath' /tmp/iso_mount" | Out-Null
        
        # Copy files (excluding large squashfs)
        Write-Log "Copying ISO contents (excluding filesystem.squashfs)..." -Level INFO
        Invoke-WslCommand -Command "rsync -a --exclude=/casper/filesystem.squashfs /tmp/iso_mount/ '$wslExtractPath/'" | Out-Null
        
        # Copy squashfs separately
        Write-Log "Copying filesystem.squashfs..." -Level INFO
        Invoke-WslCommand -Command "rsync -a /tmp/iso_mount/casper/filesystem.squashfs '$wslExtractPath/casper/'" | Out-Null
        
        # Unmount
        Write-Log "Unmounting ISO..." -Level INFO
        Invoke-WslCommand -Command "sudo umount /tmp/iso_mount" | Out-Null
        
        # Remove mount point
        Invoke-WslCommand -Command "rmdir /tmp/iso_mount" -IgnoreError | Out-Null
        ...
        Invoke-WslCommand -Command "sudo umount /tmp/iso_mount" -IgnoreError | Out-Null
        Invoke-WslCommand -Command "rmdir /tmp/iso_mount" -IgnoreError | Out-Null
...
function Update-GrubConfig {
    ...
    $fileExists = Invoke-WslCommand -Command "test -f '$grubCfgPath' && echo 'exists' || echo 'missing'"
    
    if ($fileExists.Trim() -eq 'exists') {
        Invoke-WslCommand -Command "sed -i 's|quiet splash|quiet splash $autoinstallParams|g' '$grubCfgPath'" | Out-Null
        Invoke-WslCommand -Command "sed -i 's|---\`$||g' '$grubCfgPath'" | Out-Null
    ...
    $fileExists = Invoke-WslCommand -Command "test -f '$loopbackCfgPath' && echo 'exists' || echo 'missing'"
    
    if ($fileExists.Trim() -eq 'exists') {
        Invoke-WslCommand -Command "sed -i 's|quiet splash|quiet splash $autoinstallParams|g' '$loopbackCfgPath'" | Out-Null
        Invoke-WslCommand -Command "sed -i 's|---\`$||g' '$loopbackCfgPath'" | Out-Null
...
function Copy-NocloudFiles {
    ...
    # Create nocloud directory
    Invoke-WslCommand -Command "mkdir -p '$wslExtractPath/nocloud'" | Out-Null
    
    # Copy user-data
    Invoke-WslCommand -Command "cp '$wslNocloudSource/user-data' '$wslExtractPath/nocloud/'" | Out-Null
    
    # Copy meta-data
    Invoke-WslCommand -Command "cp '$wslNocloudSource/meta-data' '$wslExtractPath/nocloud/'" | Out-Null
    
    # Set permissions
    Invoke-WslCommand -Command "chmod 644 '$wslExtractPath/nocloud/user-data'" | Out-Null
    Invoke-WslCommand -Command "chmod 644 '$wslExtractPath/nocloud/meta-data'" | Out-Null
...
function Build-CustomISO {
    ...
    try {
        ...
        $output = Invoke-WslCommand -Command $xorrisoCmd
