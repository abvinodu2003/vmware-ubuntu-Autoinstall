...
function New-SecurePasswordHash {
    ...
    $pythonCmd = "import crypt; password = '$escapedPassword'; salt = '\`$6\`$rounds=$Rounds\`$$escapedSalt\`$'; print(crypt.crypt(password, salt))"
...
function Test-Prerequisites {
    ...
    # Test sudo access and prompt if needed (ONLY ONCE)
    ...
    if ($LASTEXITCODE -ne 0) {
        ...
        Write-Host "You will be prompted for your WSL password ONCE." -ForegroundColor Cyan
        Write-Host "The password will be cached for the duration of this script." -ForegroundColor Cyan
        Write-Host ""
        
        # Prompt for sudo password and set longer timeout
        wsl -d $script:WSLDistribution bash -c "sudo -v && sudo sh -c 'echo Defaults timestamp_timeout=60 > /etc/sudoers.d/script_timeout'"
        
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to authenticate with sudo. Please check your password."
        }
        
        Write-Log "Sudo access granted and cached for 60 minutes" -Level SUCCESS
    }
    else {
...
function Extract-UbuntuISO {
    ...
    Write-Log "WSL ISO Path: $wslIsoPath" -Level INFO
    Write-Log "WSL Extract Path: $wslExtractPath" -Level INFO
    
    # Create extraction directory
    New-Item -Path $ExtractPath -ItemType Directory -Force | Out-Null
    
    # Extract ISO using separate commands with PROPER ESCAPING for paths with spaces
    try {
        # Create mount point
        Write-Log "Creating temporary mount point..." -Level INFO
        wsl -d $script:WSLDistribution bash -c "mkdir -p /tmp/iso_mount"
        
        # Mount ISO - Escape quotes for bash: we need \" inside the bash command
        Write-Log "Mounting ISO..." -Level INFO
        # The key is to escape the inner quotes so bash sees them
        $mountCmd = "sudo mount -o loop \`"$wslIsoPath\`" /tmp/iso_mount"
        wsl -d $script:WSLDistribution bash -c $mountCmd
        
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to mount ISO (exit code: $LASTEXITCODE)"
        }
        
        # Verify mount succeeded
        $mountCheck = wsl -d $script:WSLDistribution bash -c "ls /tmp/iso_mount | wc -l"
        if ([int]$mountCheck -eq 0) {
            throw "ISO mounted but appears empty"
        }
        
        Write-Log "ISO mounted successfully, found $mountCheck items" -Level SUCCESS
        
        # Copy files (excluding large squashfs) - Escape quotes for bash
        Write-Log "Copying ISO contents (excluding filesystem.squashfs)..." -Level INFO
        $rsyncCmd1 = "rsync -a --exclude=/casper/filesystem.squashfs /tmp/iso_mount/ \`"$wslExtractPath/\`""
        wsl -d $script:WSLDistribution bash -c $rsyncCmd1
        
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to copy ISO contents (exit code: $LASTEXITCODE)"
        }
        
        # Copy squashfs separately - Escape quotes for bash
        Write-Log "Copying filesystem.squashfs..." -Level INFO
        $rsyncCmd2 = "rsync -a /tmp/iso_mount/casper/filesystem.squashfs \`"$wslExtractPath/casper/\`""
        wsl -d $script:WSLDistribution bash -c $rsyncCmd2
        
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to copy filesystem.squashfs (exit code: $LASTEXITCODE)"
        }
...
function Update-GrubConfig {
    ...
    # Define the autoinstall boot parameters (no backslash escape needed)
    ...
    # Check if file exists - use escaped quotes
    ...
    if ($fileCheck.Trim() -eq 'exists') {
        ...
        Write-Log "boot/grub/loopback.cfg not found" -Level INFO
    }
}
...
function Build-CustomISO {
    ...
    # Build ISO with xorriso - use escaped quotes (backtick-quote)
    try {
        Write-Log "Running xorriso to build bootable ISO..." -Level INFO
        
        # Single command with proper escaping
...
function Invoke-Main {
    ...
    try {
        ...
        # Step 1: Validate prerequisites (includes sudo password prompt ONCE)
        Test-Prerequisites
