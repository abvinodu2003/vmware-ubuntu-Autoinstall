...
function Build-CustomISO {
    ...
    try {
        ...
        if ($efiPath) {
            # Build ISO with EFI support
            Write-Log "Building UEFI bootable ISO with: $efiPath" -Level INFO
            
            $xorrisoCmd = "cd \`"$wslExtractPath\`" && xorriso -as mkisofs -r -V 'Ubuntu-Autoinstall' -o \`"$wslOutputIsoPath\`" -J -joliet-long -eltorito-alt-boot -e $efiPath -no-emul-boot -isohybrid-gpt-basdat . 2>&1"
        }
        else {
        ...
        Write-Log "xorriso exit code: $xorrisoExitCode" -Level INFO
        ...
        if (-not [string]::IsNullOrWhiteSpace($output)) {
            ...
            if ($outputLength -gt 0) {
                ...
                # If output is longer than preview, indicate there's more
        ...
        Write-Log "Custom ISO created successfully: $('{0:N2}' -f $isoSize) GB" -Level SUCCESS
