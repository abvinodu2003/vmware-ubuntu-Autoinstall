...
.PARAMETER Debug
    Enable debug logging with detailed command output

.EXAMPLE
    # Create 5 VMs with debug logging
...
function Write-Log {
    ...
    if ($Level -eq 'DEBUG' -and -not $script:DebugEnabled) {
        return
    }
...
#region WSL Helper Functions

function Invoke-WslCommand {
    <#
    .SYNOPSIS
        Executes a command in WSL with proper error handling
    #>
    param(
        [Parameter(Mandatory = $true)]
        [string]$Command,
    ...
    try {
        ...
        $cleanCommand = $Command -replace "`r`n", "`n" -replace "`r", ""
        
        # Log command in debug mode
        if ($script:DebugEnabled) {
            Write-Log "WSL Command: $cleanCommand" -Level DEBUG
        }
        
        # Execute command with dynamic distribution
        if ($script:WSLDistribution) {
            $result = wsl -d $script:WSLDistribution bash -c $cleanCommand 2>&1
        }
        else {
            $result = wsl bash -c $cleanCommand 2>&1
        }
        
        # Log result in debug mode
        if ($script:DebugEnabled) {
            Write-Log "WSL Exit Code: $LASTEXITCODE" -Level DEBUG
            if ($result) {
                Write-Log "WSL Output: $result" -Level DEBUG
            }
        }
        ...
        if (-not $IgnoreError) {
            Write-Log "WSL Command Error: $_" -Level DEBUG
            throw
        }
        return $null
    }
...
function New-SecurePasswordHash {
    ...
    Write-Log "Generated salt: $salt" -Level DEBUG
    
    # Escape special characters for bash - use double quotes to avoid most issues
    ...
    $pythonCmd = "import crypt; print(crypt.crypt(`"$escapedPassword`", `"\`$6\`$rounds=$Rounds\`$$escapedSalt\`$`"))"
    
    try {
        # First, ensure Python3 is available in WSL
        $pythonCheck = Invoke-WslCommand -Command "which python3" -IgnoreError
        
        if ([string]::IsNullOrWhiteSpace($pythonCheck)) {
            Write-Log "Python not found in WSL, installing..." -Level WARNING
            Invoke-WslCommand -Command "sudo apt-get update -qq && sudo apt-get install -y -qq python3" | Out-Null
        }
        
        # Generate the hash
        $hashResult = Invoke-WslCommand -Command "python3 -c `"$pythonCmd`""
        ...
        Write-Log "Generated hash: $($hashResult.Trim())" -Level DEBUG
        ...
        try {
            ...
            Invoke-WslCommand -Command "command -v mkpasswd >/dev/null 2>&1 || sudo apt-get install -y -qq whois" -IgnoreError | Out-Null
            
            # Generate hash using mkpasswd
            $mkpasswdCmd = "echo '$escapedPassword' | mkpasswd -m sha-512 -R $Rounds -S '$escapedSalt' -s"
            $hashResult = Invoke-WslCommand -Command $mkpasswdCmd
...
function Test-Prerequisites {
    ...
    Write-Progress -Activity "Validating Environment" -Status "Checking sudo access..." -PercentComplete 40
    
    # Test sudo access and prompt if needed
    Write-Host "`n------------------------------------------------------------------" -ForegroundColor Yellow
    Write-Host "  WSL Sudo Access Required" -ForegroundColor Yellow
    Write-Host "------------------------------------------------------------------`n" -ForegroundColor Yellow
    
    # Quick sudo check
    $sudoCheck = wsl -d $script:WSLDistribution bash -c "sudo -n true 2>&1"
    
    if ($LASTEXITCODE -ne 0) {
        Write-Host "The script requires sudo access in WSL to install packages and mount ISOs." -ForegroundColor White
        Write-Host "Please enter your WSL Ubuntu password when prompted below..." -ForegroundColor Cyan
        Write-Host ""
        
        # Prompt for sudo password
        wsl -d $script:WSLDistribution bash -c "sudo true"
        
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to authenticate with sudo. Please check your password."
        }
    }
    
    Write-Log "Sudo access verified" -Level SUCCESS
    ...
    # Single fast command
    try {
        wsl -d $script:WSLDistribution bash -c "sudo apt-get update -qq && sudo apt-get install -y -qq $packageList"
...
function Initialize-WorkingDirectory {
    ...
    if (-not (Test-Path -LiteralPath $OutputDirectory)) {
        Write-Log "Creating output directory: $OutputDirectory" -Level DEBUG
...
function ConvertTo-WslPath {
    ...
    Write-Log "Converting Windows path to WSL: $WindowsPath" -Level DEBUG
    ...
    $result = $wslPath.Trim()
    Write-Log "WSL path: $result" -Level DEBUG
...
function New-AutoinstallConfig {
    ...
    Write-Log "Creating autoinstall config for VM: $VMName" -Level DEBUG
    Write-Log "  Username: $Username" -Level DEBUG
    Write-Log "  Hostname: $Hostname" -Level DEBUG
...
function Extract-UbuntuISO {
    ...
    # Extract ISO using separate commands (FAST - no heredoc)
    try {
        # Create mount point
        Write-Log "Creating temporary mount point..." -Level INFO
        ...
        # Copy files (excluding large squashfs)
        Write-Log "Copying ISO contents (excluding filesystem.squashfs)..." -Level INFO
        wsl -d $script:WSLDistribution bash -c "rsync -a --exclude=/casper/filesystem.squashfs /tmp/iso_mount/ '$wslExtractPath/'"
        
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to copy ISO contents"
        }
        
        # Copy squashfs separately
        Write-Log "Copying filesystem.squashfs..." -Level INFO
        wsl -d $script:WSLDistribution bash -c "rsync -a /tmp/iso_mount/casper/filesystem.squashfs '$wslExtractPath/casper/'"
        
        if ($LASTEXITCODE -ne 0) {
            throw "Failed to copy filesystem.squashfs"
        }
        ...
        # Cleanup on error
        Write-Log "Cleaning up after extraction error..." -Level WARNING
...
function Update-GrubConfig {
    ...
    # Define the autoinstall boot parameters (no backslash escape)
    $autoinstallParams = 'autoinstall ds=nocloud;s=/cdrom/nocloud/ ---'
    
    # Update boot/grub/grub.cfg
    Write-Log "Updating boot/grub/grub.cfg..." -Level INFO
    ...
    if ($fileCheck.Trim() -eq 'exists') {
        ...
        Write-Log "boot/grub/grub.cfg not found, skipping..." -Level WARNING
    }
    
    # Update boot/grub/loopback.cfg if present
    Write-Log "Checking for boot/grub/loopback.cfg..." -Level INFO
    ...
    if ($fileCheck.Trim() -eq 'exists') {
        ...
        Write-Log "boot/grub/loopback.cfg not found, skipping..." -Level INFO
    }
}
...
function Copy-NocloudFiles {
    ...
    Write-Log "NoCloud files copied successfully" -Level SUCCESS
}
...
function Build-CustomISO {
    ...
    # Build ISO with xorriso (one-liner, no heredoc)
    try {
        Write-Log "Running xorriso to build bootable ISO..." -Level INFO
        
        # Single command - much faster than heredoc
        $xorrisoCmd = "cd '$wslExtractPath' && xorriso -as mkisofs -r -V 'Ubuntu 24.04 Autoinstall' -o '$wslOutputIsoPath' -J -joliet-long -cache-inodes -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -eltorito-alt-boot -e boot/grub/efi.img -no-emul-boot -isohybrid-gpt-basdat -isohybrid-apm-hfsplus -isohybrid-mbr /usr/lib/ISOLINUX/isohdpfx.bin . 2>&1"
        
        $output = wsl -d $script:WSLDistribution bash -c $xorrisoCmd
        
        if ($LASTEXITCODE -ne 0) {
            Write-Log "xorriso output: $output" -Level ERROR
...
function Create-VMwareVM {
    ...
    Write-Log "  CPU: $CpuCount cores" -Level DEBUG
    Write-Log "  Memory: $MemoryMB MB" -Level DEBUG
    Write-Log "  Disk: $DiskSizeGB GB" -Level DEBUG
    Write-Log "  Network: $NetworkType" -Level DEBUG
    ...
    Write-Log "Created VM directory: $vmDir" -Level DEBUG
...
"@
    ...
    Write-Log "Created VMX file: $vmxPath" -Level DEBUG
    ...
    if (Test-Path -LiteralPath $vdiskManager) {
        Write-Log "Using vmware-vdiskmanager to create disk..." -Level DEBUG
        $createDiskCmd = "& `"$vdiskManager`" -c -s ${DiskSizeGB}GB -a lsilogic -t 0 `"$vmdkPath`" 2>&1"
        $null = Invoke-Expression $createDiskCmd
    }
    else {
        Write-Log "vmware-vdiskmanager not found, creating VMDK descriptor..." -Level WARNING
...
function Start-VMwareVM {
    ...
    try {
        ...
        Write-Log "Start command: $startCmd" -Level DEBUG
        
        $output = Invoke-Expression $startCmd
        
        if ($script:DebugEnabled -and $output) {
            Write-Log "vmrun output: $output" -Level DEBUG
        }
...
function Build-VMConfiguration {
    ...
    try {
        ...
        Write-Log "[$vmName] VM extract path: $vmExtractPath" -Level DEBUG
...
function Start-ParallelVMDeployment {
    ...
    Write-Log "Starting parallel VM deployment for $totalVMs VMs..." -Level DEBUG
    ...
    foreach ($vmConfig in $VMConfigurations) {
        ...
        while ($runningJobs.Count -ge $MaxParallelVMs) {
            ...
            Write-Log "Waiting for VM slots (Running: $($runningJobs.Count))" -Level DEBUG
        }
        ...
        Write-Log "Creating background job for $vmName..." -Level DEBUG
...
function Invoke-Main {
    ...
    try {
        ...
        if ($script:DebugEnabled) {
            Write-Host "  DEBUG MODE ENABLED - Detailed logging active" -ForegroundColor Yellow
            Write-Host "==================================================================`n" -ForegroundColor Cyan
        }
        ...
        # Step 1: Validate prerequisites (this will set $script:WSLDistribution)
        Test-Prerequisites
        ...
        Write-Log "Debug Mode: $script:DebugEnabled" -Level INFO
        ...
        for ($i = 0; $i -lt $VMCount; $i++) {
            ...
            Write-Log "Configuring VM $vmName (Username: $username, Hostname: $hostname)" -Level DEBUG
