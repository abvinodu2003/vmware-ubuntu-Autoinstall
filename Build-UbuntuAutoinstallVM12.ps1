...
function Build-CustomISO {
    ...
    Write-Log "Building bootable ISO for $VMName (preserving boot structure)..." -Level INFO
    ...
    try {
        ...
        # Check for boot.catalog (critical for bootability)
        $bootCatalog = wsl -d $script:WSLDistribution bash -c "test -f '$wslExtractPath/boot.catalog' && echo exists || echo missing"
        $hasBootCatalog = $bootCatalog.Trim() -eq 'exists'
        
        Write-Log "UEFI boot (EFI/boot/bootx64.efi): $hasUefiBoot" -Level INFO
        Write-Log "BIOS boot (isolinux/isolinux.bin): $hasBiosBoot" -Level INFO
        Write-Log "Boot catalog (boot.catalog): $hasBootCatalog" -Level INFO
        
        # Add BIOS boot if missing
        if (-not $hasBiosBoot) {
            Write-Log "Adding BIOS boot support for hybrid compatibility..." -Level INFO
            
            wsl -d $script:WSLDistribution bash -c "mkdir -p '$wslExtractPath/isolinux'"
            
            # Copy isolinux files
            $isolinuxCopied = $false
            ...
            foreach ($path in $isolinuxPaths) {
                $exists = wsl -d $script:WSLDistribution bash -c "test -f '$path' && echo yes || echo no"
                if ($exists.Trim() -eq 'yes') {
                    wsl -d $script:WSLDistribution bash -c "cp '$path' '$wslExtractPath/isolinux/isolinux.bin'"
                    $isolinuxCopied = $true
                    Write-Log "Copied isolinux.bin from: $path" -Level SUCCESS
                    break
                }
            }
            ...
            if ($isolinuxCopied) {
                ...
                wsl -d $script:WSLDistribution bash -c "cp /usr/lib/syslinux/modules/bios/*.c32 '$wslExtractPath/isolinux/' 2>/dev/null || true"
...
'@
        ...
        Write-Progress -Activity "Building ISO" -Status "Building bootable ISO..." -PercentComplete 30
        
        $startTime = Get-Date
        
        # CRITICAL: Use the EXACT xorriso command that works
        # This mimics what VirtualBox does - preserving boot sectors
        
        if ($hasBiosBoot -and $hasUefiBoot) {
            Write-Log "Building HYBRID BIOS+UEFI bootable ISO" -Level SUCCESS
            
            # Find MBR template
            $mbrTemplate = wsl -d $script:WSLDistribution bash -c "find /usr -name 'isohdpfx.bin' 2>/dev/null | head -1"
            $mbrTemplate = $mbrTemplate.Trim()
            
            if ([string]::IsNullOrWhiteSpace($mbrTemplate)) {
                Write-Log "MBR template not found, using basic hybrid mode" -Level WARNING
...
"@
                ...
                Write-Log "Using MBR template: $mbrTemplate" -Level INFO
...
"@
        ...
        # Show xorriso output
        if ($output) {
            Write-Log "xorriso output (first 15 lines):" -Level INFO
            $outputLines = $output -split "`n"
            $lineCount = [Math]::Min(15, $outputLines.Count)
        ...
        Write-Log "Bootable ISO created: $('{0:N2}' -f $isoSize) GB" -Level SUCCESS
        
        if ($hasBiosBoot -and $hasUefiBoot) {
            Write-Log "✓ ISO is HYBRID bootable (BIOS + UEFI)" -Level SUCCESS
        } elseif ($hasUefiBoot) {
            Write-Log "✓ ISO is UEFI bootable" -Level SUCCESS
        }
